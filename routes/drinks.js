...
.get(isLoggedIn, (req, res) => {
  ...
  Drink.findById(id).then((drink) => res.render("drinks/edit-drink", drink));
})
.post(fileUploader.single("image"), (req, res) => {
  ...
  Drink.findByIdAndUpdate(
    id,
    { name, description, origin, alcohol_content, image },
    { new: true }
  )
  .then(() => res.redirect("/drinks"))
  .catch((error) =>
      console.log("The edit drink didnt work becasue: ", error)
  );
});
    ...
    .then((newBeer) => {
      User.findById(req.session.userId)
        ...
        .then((user) => {
          ...
          if (flag === false) {
            favs.push(newBeer);
            user.save().then(() => res.send(newBeer.likes.toString()));
          }
        });
    })
...
router
  ...
  .post(fileUploader.single("image"), (req, res) => {
    ...
    if (req.file) {
      ...
      image =
        "https://upload.wikimedia.org/wikipedia/commons/thumb/d/db/Birra_Moretti_Logo_2015.jpeg/640px-Birra_Moretti_Logo_2015.jpeg";
    }
    Drink.create({
      name: name1,
      ...
      .then(res.redirect("/drinks"))
      .catch((error) =>
        console.log("The create drink didnt work becasue: ", error)
      );
  });
